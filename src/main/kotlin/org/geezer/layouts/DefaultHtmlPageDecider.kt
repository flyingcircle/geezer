...
class DefaultHtmlPageDecider : UseLayoutDecider {
    ...
    companion object {
        ...
        fun knownNonHtmlFile(uri: String): Boolean {
            val extensionIndex = uri.lastIndexOf('.')
            return if (extensionIndex > 0) {
                val lastSlashIndex = uri.lastIndexOf('/')
                if (extensionIndex > lastSlashIndex && extensionIndex < uri.length) {
                    val extension = uri.substring(extensionIndex + 1).lowercase()
                    knownNonHtmlFileExtensions.contains(extension)
                } else {
                    false
                }
            } else {
        ...
        val knownNonHtmlFileExtensions = listOf(
