...
internal object RouteCriteriaBuilder {
    fun build(tokens: RouteTokens, function: RouteFunction, context: RouteContext, configuration: RoutesConfiguration): RouteCriteria {
        ...
        for ((index, token) in tokens.pathTokens.withIndex()) {
            when (token) {
                is ExactValuePathToken -> {
                    ...
                    val parameterTypeFromRoute = if (numberPotentialRouteParameters < parameterTypesFromRoutes.size) parameterTypesFromRoutes[numberPotentialRouteParameters] else null
                    if (parameterTypeFromRoute != null && parameterTypeFromRoute.optional) {
                        throw IllegalArgumentException("Route function $function has unsupported optional path parameter ${parameterTypeFromRoute.clazz.clazz.qualifiedName} at index $numberPotentialRouteParameters.")
                    }
                    ...
                    when (token) {
                        ...
                        is PatternPathToken -> {
                            try {
                                val criterion = PathCriterion(index, PathCriterionType.Regex, token.pattern, Regex(token.pattern))
                                pathCriteria.add(criterion)
                                numberPotentialRouteParameters += criterion.numberPatternGroups.coerceAtLeast(1)
                            } catch (e: PatternSyntaxException) {
                                throw IllegalArgumentException("Route function $function has invalid path regular expression pattern ${token.pattern}.", e)
                            }
                        }

                        is FunctionParameterPathToken -> {
                            val regex = parameterTypesFromRoutes.getOrNull(numberPotentialRouteParameters)?.clazz?.routeRegex ?: throw IllegalArgumentException("Route function $function has path pattern {} with no matching parameter.")
                            pathCriteria.add(PathCriterion(index, PathCriterionType.Regex, regex.pattern, regex))
                        }

                        is SymbolPathToken -> {
                            val regex = configuration.symbolsToRegexs[token.symbol] ?: throw IllegalArgumentException("Route function $function references undefined symbol ${token.symbol} in path token.")
                            pathCriteria.add(PathCriterion(index, PathCriterionType.Regex, regex.pattern, regex))
                        }
        ...
        for (token in tokens.parameterTokens) {
            when (token) {
                is ExactValueParameterToken -> {
                    parameterCriteria.add(ParameterCriterion(token.name, ParameterCriterionType.Exact, token.value, token.optional))
                }
                    ...
                    when (token) {
                        is WildcardParameterToken -> {
                            parameterCriteria.add(ParameterCriterion(token.name, ParameterCriterionType.Regex, "*", optional, WildCardRegex))
                        }

                        is PatternParameterToken -> {
                            try {
                                val criterion = ParameterCriterion(token.name, ParameterCriterionType.Regex, token.pattern, optional, token.pattern.toRegex())
                                parameterCriteria.add(criterion)
                                numberPotentialRouteParameters += criterion.numberPatternGroups.coerceAtLeast(1)
                            } catch (e: PatternSyntaxException) {
                                throw IllegalArgumentException("Route function $function has invalid parameter regular expression pattern ${token.pattern}.", e)
                            }
                        }

                        is FunctionParameterParameterToken -> {
                            val regex = parameterTypesFromRoutes.getOrNull(numberPotentialRouteParameters)?.clazz?.routeRegex ?: throw IllegalArgumentException("Route function $function has parameter pattern {} with no matching parameter.")
                            parameterCriteria.add(ParameterCriterion(token.name, ParameterCriterionType.Regex, regex.pattern, optional, regex))
                        }

                        is SymbolParameterToken -> {
                            val regex = configuration.symbolsToRegexs[token.symbol] ?: throw IllegalArgumentException("Route function $function references undefined symbol ${token.symbol} in parameter token.")
                            parameterCriteria.add(ParameterCriterion(token.name, ParameterCriterionType.Regex, regex.pattern, optional, regex))
                        }
        ...
        val methods = context.methods.toMutableList()
        if (methods.isEmpty()) {
            methods.add(getMethodFromFunction(function))
        }
